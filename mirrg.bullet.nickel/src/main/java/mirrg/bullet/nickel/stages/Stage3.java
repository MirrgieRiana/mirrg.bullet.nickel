...
import mirrg.bullet.nickel.weapons.WeaponLaserRuby;
import mirrg.bullet.nickel.weapons.WeaponScatterCalcite;
import mirrg.bullet.nickel.weapons.WeaponScatterLaserRuby;
...
public class Stage3 implements IStage
{
	...
	private void lrlr(PhaseBattle phase)
	{
		for (int j = 0; j < 2; j++) {
			...
			for (int i = 0; i < 10; i++) {
				if (age == A) {
					phase.addEnemy(new EnemyFairy1(0, 0.15, 0.005, 0, 25)
			...
			for (int i = 0; i < 10; i++) {
				if (age == A) {
					phase.addEnemy(new EnemyFairy1(1, 0.15, -0.005, 0, 25)
						.addWeapon(new WeaponDelay(50, new WeaponBulletsNickel(0)))
						.addStack(() -> new StackItem(Items.nickel, 1), 0.1));
				}
				A += 10;
	...
	private void lrDouji(PhaseBattle phase)
	{
		for (int i = 0; i < 10; i++) {
			if (age == A) {
				phase.addEnemy(new EnemyFairy1(0, 0.25, 0.005, 0, 100)
					.addWeapon(new WeaponDelay(50, new WeaponBulletsNickel(0)))
					.addStack(() -> new StackItem(Items.nickel, 1), 0.1));
				phase.addEnemy(new EnemyFairy1(1, 0.35, -0.005, 0, 100)
					.addWeapon(new WeaponDelay(50, new WeaponBulletsNickel(0)))
					.addStack(() -> new StackItem(Items.nickel, 1), 0.1));
			}
			A += 10;
		...
		for (int i = 0; i < 10; i++) {
			if (age == A) {
				phase.addEnemy(new EnemyFairy1(0, 0.45, 0.005, 0, 100)
					.addWeapon(new WeaponDelay(50, new WeaponBulletsNickel(0)))
					.addStack(() -> new StackItem(Items.nickel, 1), 0.1));
				phase.addEnemy(new EnemyFairy1(1, 0.55, -0.005, 0, 100)
					.addWeapon(new WeaponDelay(50, new WeaponBulletsNickel(0)))
					.addStack(() -> new StackItem(Items.nickel, 1), 0.1));
			}
			A += 10;
	...
	private void issei(PhaseBattle phase)
	{
		for (int i = 0; i < 5; i++) {
			if (age == A) {
				for (int j = 0; j < 10; j++) {
					phase.addEnemy(new EnemyFairy1(0.1 + 0.08 * j, 0, 0, 0.005, 25)
