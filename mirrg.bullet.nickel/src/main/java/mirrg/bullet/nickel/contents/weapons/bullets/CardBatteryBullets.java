...
import mirrg.bullet.nickel.weapon.card.KeyBoolean;
import mirrg.bullet.nickel.weapon.card.KeyColor;
import mirrg.bullet.nickel.weapon.card.KeyDouble;
import mirrg.bullet.nickel.weapon.instance.IBattery;

public class CardBatteryBullets extends CardBatteryAbstract
{
	...
	public static final KeyDouble ANGLE_PER_BULLET = new KeyDouble("ANGLE_PER_BULLET", 0);
	...
	public CardBatteryBullets(CardBatteryAbstract... parents)
	{
		super(parents);
	}
	...
	public IBattery createBattery()
	{
		return new IBattery() {
			...
			private void fire(ILiving living, PhaseBattle phase, boolean isPlayer, double extra)
			{
				...
				for (int w = 0; w < ways; w++) {
					...
					for (int b = 0; b < bullets; b++) {
						...
						speed *= 1 + w * get(SPEED_RATE_PER_WAY);
	...
	double getAngle(double x, double y, PhaseBattle phase, boolean isPlayer)
	{
		if (get(SEARCH)) {
			ILiving nearest;
			if (isPlayer) {
